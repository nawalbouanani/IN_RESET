...
const NetworkAnimation = () => {
  ...
  const config = {
    nodeCount: 200, // más nodos para intensidad
    maxDistance: 140, // un poco más de distancia para conexiones
    nodeSize: 3, // nodos un poco más grandes
    speed: 0.004,
    mouseInfluence: 300,
  ...
  class Node {
    ...
    reset() {
      // Concentrar nodos en el 50% izquierdo horizontalmente, pero verticalmente todo el canvas
    ...
    update(time) {
      ...
      // Si nodo se sale mucho, reiniciar dentro del área izquierda para mantener la concentración
      if (
  this.z < -config.fov ||
  projectedX < -50 ||
  ...
  // Nota: reset() ya coloca x en todo el ancho, no limita al centro
}
  ...
  useEffect(() => {
    ...
    function animate(time) {
      ...
      // Dibujar conexiones limitando a 7 conexiones por nodo (más densidad)
      for (let i = 0; i < nodesRef.current.length; i++) {
        let connectionsCount = 0;
        for (let j = i + 1; j < nodesRef.current.length; j++) {
          if (connectionsCount >= 7) break;
